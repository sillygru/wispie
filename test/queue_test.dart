...
void main() {
  ...
  group('Priority Block Logic Simulation', () {
    ...
    test('Shuffle should keep priority items after current', () {
      ...
      List<QueueItem> effectiveQueue = songs.map((s) => QueueItem(song: s)).toList();
      
      // Current song is at index 0 (Song 0)
      final currentItem = effectiveQueue[0];
      
      // Add a priority song at index 1
      final prioritySong = Song(title: 'Priority', artist: 'A', album: 'A', filename: 'p.mp3', url: 'u');
      ...
      // Simulate shuffle
      final otherItems = effectiveQueue.where((item) => item != currentItem).toList();
