...
double calculateWeight(
  ...
  if (config.personality == ShufflePersonality.defaultMode) {
    ...
    if (config.antiRepeatEnabled && shuffleState.history.isNotEmpty) {
      int historyIndex = shuffleState.history
...
void main() {
  final songNew = Song(title: 'New', artist: 'A', album: 'X', filename: 'new.mp3', url: '', playCount: 0);
  final songFrequent = Song(title: 'Frequent', artist: 'A', album: 'X', filename: 'freq.mp3', url: '', playCount: 60);
  final songFav = Song(title: 'Fav', artist: 'B', album: 'Y', filename: 'fav.mp3', url: '', playCount: 20);
  ...
  group('Personality: EXPLORER', () {
    final config = const ShuffleConfig(personality: ShufflePersonality.explorer);
    final state = ShuffleState(config: config);

    test('Explorer gives massive boost to unplayed songs', () {
      final weight = calculateWeight(itemNew, null, state, [], [], {'new.mp3': 0});
      expect(weight, equals(50.0));
    });

    test('Explorer penalizes overplayed songs', () {
      final weight = calculateWeight(itemFreq, null, state, [], [], {'freq.mp3': 60});
      expect(weight, equals(0.01));
    });

    test('Explorer still applies anti-repeat', () {
      final stateWithHistory = state.copyWith(history: [HistoryEntry(filename: 'new.mp3', timestamp: 100)]);
      final weight = calculateWeight(itemNew, null, stateWithHistory, [], [], {'new.mp3': 0});
      // 50.0 * (1.0 - 0.95) = 2.5
  ...
  group('Personality: CONSISTENT', () {
    final config = const ShuffleConfig(personality: ShufflePersonality.consistent);
    final state = ShuffleState(config: config);

    test('Consistent gives boost to favorites', () {
      final weight = calculateWeight(itemFav, null, state, ['fav.mp3'], [], {'fav.mp3': 20});
    ...
    test('Consistent has relaxed anti-repeat (only last 10)', () {
      ...
      final stateRecent = state.copyWith(history: List.generate(5, (i) => HistoryEntry(filename: 'other$i.mp3', timestamp: 1.0))..add(HistoryEntry(filename: 'fav.mp3', timestamp: 0.0)));
      final weightRecent = calculateWeight(itemFav, null, stateRecent, [], [], {'fav.mp3': 20});
      // playCount > 10 (1.5) * anti-repeat (0.05) = 0.075
      expect(weightRecent, closeTo(0.075, 0.001));

      // Index 15 in history (greater than 10)
      final stateOld = state.copyWith(history: List.generate(15, (i) => HistoryEntry(filename: 'other$i.mp3', timestamp: 1.0))..add(HistoryEntry(filename: 'fav.mp3', timestamp: 0.0)));
      final weightOld = calculateWeight(itemFav, null, stateOld, [], [], {'fav.mp3': 20});
