...
void main() {
  group('Selection Sync Edge Cases', () {
    ...
    test('Selecting a song when shuffle is ON moves it to front', () {
      ...
      final originalIdx = originalQueue
          .indexWhere((item) => item.song.filename == selectedSong.filename);
      final selectedItem = originalQueue[originalIdx];

      final otherItems = List<QueueItem>.from(originalQueue)
        ..removeAt(originalIdx);
      // In shuffle mode, we expect effectiveQueue[0] to be the selected song
      final effectiveQueue = [
        selectedItem,
        ...otherItems
      ]; // simplified shuffle for test
