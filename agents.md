...
- **Comments**: Keep comments minimal. Explain _why_, not _what_. Do not talk to yourself during comments. Do not add LLM specific comments such as "// added this line" or "// per user request", do not add too many comments per function, explain function at beginning then fully implement it with no more comments.
- **Emojis**: Do _NOT_ _EVER_ use emojis in code, code comments or messages.
- **No building**: Do _NOT_ _EVER_ build the project _UNLESS_ explicitly requested.
