...
class _CacheManagementScreenState extends State<CacheManagementScreen> {
  ...
  Future<void> _calculateSizes() async {
    ...
    try {
      int songsSize =
          await CacheService.instance.getCacheSize(category: 'songs');
      int imagesSize =
          await CacheService.instance.getCacheSize(category: 'images');
      int lyricsSize =
          await CacheService.instance.getCacheSize(category: 'lyrics');
  ...
  void _showCategoryDetails(String category, String title) {
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) =>
            CacheCategoryDetailScreen(category: category, title: title),
      ),
              ...
              children: [
                ...
                _buildActionTile(
                  ...
                  onClear: () => _performClear(() =>
                      CacheService.instance.clearCache(category: 'songs')),
                ),
                _buildActionTile(
                  title: "Image Cache",
                  ...
                  onClear: () => _performClear(() =>
                      CacheService.instance.clearCache(category: 'images')),
                ),
                _buildActionTile(
                  title: "Lyrics Cache",
                  ...
                  onClear: () => _performClear(() =>
                      CacheService.instance.clearCache(category: 'lyrics')),
                ),
                  ...
                  onTap: () async {
                    final confirm = await showDialog<bool>(
                      ...
                      builder: (context) => AlertDialog(
                        ...
                        content: const Text(
                            "This will delete all cached data and pause downloads for 10 seconds."),
                        actions: [
                          TextButton(
  ...
  Widget _buildSummaryCard() {
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            const Text("Total Cache Used",
                style: TextStyle(fontSize: 16, fontWeight: FontWeight.w500)),
            const SizedBox(height: 8),
            Text(_totalCacheSize,
                style:
                    const TextStyle(fontSize: 32, fontWeight: FontWeight.bold)),
            if (CacheService.instance.isPaused)
              const Padding(
                padding: EdgeInsets.only(top: 8.0),
                child: Text("Downloads Paused",
                    style: TextStyle(
...
class CacheCategoryDetailScreen extends StatefulWidget {
  ...
  const CacheCategoryDetailScreen(
      {super.key, required this.category, required this.title});

  @override
  State<CacheCategoryDetailScreen> createState() =>
      _CacheCategoryDetailScreenState();
}
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  ...
                  subtitle: Text(
                      "Validated: ${entry.lastValidated.toString().split('.')[0]}"),
                  trailing: IconButton(
                    icon: const Icon(Icons.delete, color: Colors.grey),
                    onPressed: () async {
                      await CacheService.instance
                          .removeEntry(widget.category, key);
                      setState(() {});
                    },
                  ),
                );
