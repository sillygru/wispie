    ...
    if (playlistId == '__favorites__') {
      playlist = Playlist(
        ...
        songs: userData.favorites
            .map((f) => PlaylistSong(filename: f, addedAt: DateTime.now()))
            .toList(),
      );
    } else {
      playlist = userData.playlists.firstWhere((p) => p.id == playlistId,
          orElse: () => const Playlist(id: '', name: 'Not Found', songs: []));
    }
          ...
          songsAsync.when(
            data: (allSongs) {
              ...
              for (final ps in playlist.songs) {
                final songIndex =
                    allSongs.indexWhere((s) => s.filename == ps.filename);
                if (songIndex != -1) {
              ...
              data: (allSongs) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final addedDate =
                        "${playlistSong.addedAt.day}/${playlistSong.addedAt.month}/${playlistSong.addedAt.year}";
                    final isFavorite =
